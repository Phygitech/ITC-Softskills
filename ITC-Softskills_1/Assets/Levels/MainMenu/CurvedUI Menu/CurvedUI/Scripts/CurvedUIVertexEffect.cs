    ...
    public partial class CurvedUIVertexEffect : BaseMeshEffect
    {
        ...
        if (myTMP || myTMPSubMesh) return;
            ...
            { // do not perform tesselation required check if we already know it is, god damnit!
                ...
                {//test for color changes if it has a graphic component
            ...
            { // do not perform a check if we're already tesselating or curving. Tesselation includes curving.
            ...
            if (myGraphic && (tesselationRequired || curvingRequired))
                myGraphic.SetVerticesDirty();
        }
        ...
        void ModifyVerts(List<UIVertex> verts)
        {
            ...
            { // use saved verts if we have those
                ...
                curvedVerts = new Vector3[tesselatedVerts.Count];

                for (int i = 0; i < tesselatedVerts.Count; i++)
                {
                    copiedVerts[i] = CurveVertex(tesselatedVerts[i], angle, radius, canvasSize);

                    curvedVerts[i] = copiedVerts[i].position;
                    Vector3 pos = tesselatedVerts[i].position;
            ...
            { // or just the mesh's vertices if we do not
                ...
                curvedVerts = new Vector3[verts.Count];
        ...
        {
            m = new Mesh();
            m.vertices = curvedVerts;

            List<int> tris = new List<int>();
            for (int i = 0; i < curvedVerts.Length / 4; i++)
            {
                tris.Add(i * 4 + 0);
                tris.Add(i * 4 + 1);
                tris.Add(i * 4 + 2);
            ...
            MeshCollider _meshC;
            _meshC = gameObject.GetComponent<MeshCollider>();

            if (_meshC == null)
                _meshC = gameObject.AddComponent<MeshCollider>();
