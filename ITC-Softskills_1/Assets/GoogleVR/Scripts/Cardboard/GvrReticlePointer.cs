...
///If true then Gaze distance from camera will not change
public static bool LockDistance = false;
[Tooltip("If true then Gaze distance will remain at its last distance if no target is present")]
public bool MaintainPreviousDistance = true;
  ...
  public void UpdateDiameters() {
    ...
    float outer_half_angle_radians = Mathf.Deg2Rad * ReticleOuterAngle * 0.5f;
		...
		float inner_diameter = 1.2f * Mathf.Tan(inner_half_angle_radians);
		float outer_diameter = 1.5f * Mathf.Tan(outer_half_angle_radians);
