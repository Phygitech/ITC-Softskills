...
public class BelowUiEnableDisable : MonoBehaviour
{
	...
	void Start ()
	{
		if (!usingFirstPerson){
			...
			transform.localEulerAngles = new Vector3 (transform.localEulerAngles.x, Camera.main.transform.parent.localEulerAngles.y, transform.localEulerAngles.z);
			transform.position = Camera.main.transform.parent.position + Camera.main.transform.parent.forward * 0.06f;
			transform.position = new Vector3 (transform.position.x, Camera.main.transform.parent.position.y - 0.05f, transform.position.z);
		}
	}
