...
public class LanguageManager : MonoBehaviour, ISerializationCallbackReceiver
{
		...
		if(LoadAvailableCultures())
		{
			...
			if(availableLanguages.cultureInfos.Count > 0)
			{
				...
				{		
                 ...
                 //   print(availableLanguages.cultureInfos[0]);
    ...
    //Trying to find the affect on langData if static keyword not use 
     //static TextAsset languageData;
        TextAsset languageData;
	/// <summary>
	/// Change the language into a specified culture
	/// </summary>
	/// <param name="languageCode">The culture to change to</param>
        ...
        {
           // Debug.Log("ChangeLanguage "+languageCode);
            if ( PlayerPrefs.GetString("currentLanguage") == languageCode )
            {
				if (languageCode == LanguageHandler.defaultLanguage ||languageCode == "hi-IN")//|| languageCode == "en-GB" || languageCode == "zh-CN"|| languageCode == "ar-AR")   
                { 
                    languageData = Resources.Load(LanguageRuntimeData.LanguageFilePath(languageCode)) as TextAsset;
                }
                else 
                {
                    try
                    {
                        ...
                        languageData = AssetBundleManager.Instance.GetAssetBundle(languageCode).LoadAsset<TextAsset>("Language." + languageCode);
                ...
                if(OnChangeLanguage != null) {
                    ...
                    Debug.Log("Sourabh-MethodItSelfIsCallingTwice");
                }
            }
