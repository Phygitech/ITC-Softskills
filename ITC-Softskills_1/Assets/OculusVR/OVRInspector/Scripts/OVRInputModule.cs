...
namespace UnityEngine.EventSystems
{
    ...
    {
		public static  OVRInputModule Instance;
        ...
        /// </summary>
        [SerializeField]
	public string m_SubmitButton = "Submit"; //rajni
		...
		public List<PointerEventData> LastPointerEvent = new List<PointerEventData> ();
        ...
        private void ProcessMousePress(MouseButtonEventData data)
        {
            ...
            if (data.PressedThisFrame() && !IsDownActive)
            {
                IsDownActive = true;
                //Debug.LogError ("ProcessMousePress "+ "PointerDown " + Time.time);
                ...
                LastPointerEvent.Add (pointerEvent);

                //Debug.LogError (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) + " " + data.buttonData);
            }

            // PointerUp notification
            if (data.ReleasedThisFrame() && IsDownActive)
            {
                IsDownActive = false;
                //Debug.LogError ("ProcessMousePress "+ " Pointerup " + Time.time);

                // Debug.Log("Executing pressup on: " + pointer.pointerPress);
                ExecuteEvents.Execute(pointerEvent.pointerPress, pointerEvent, ExecuteEvents.pointerUpHandler);

                //Debug.Log("KeyCode: " + data.buttonData.eventData.keyCode);
                ...
                if(LastPointerEvent.Count > 0)
                    LastPointerEvent.RemoveAt (LastPointerEvent.Count - 1);
                if (LastPointerEvent.Count > 0) {
                    foreach (PointerEventData eventData in LastPointerEvent) {
                        //Debug.LogError(LastPointerEvent.Count + " " + eventData.lastPress);
                        ExecuteEvents.Execute (eventData.lastPress, eventData, ExecuteEvents.pointerUpHandler);
                    }
                    LastPointerEvent.Clear ();
                }

            }

			//StartCoroutine ("DelayMouseEvent", data);
        }


		IEnumerator DelayMouseEvent(MouseButtonEventData data) {
            ...
            if (data.PressedThisFrame() )
			{
                ...
                if (OVRInput.GetDown(OVRInput.Touch.PrimaryTouchpad)) {
                    //Debug.LogError ("1 GetDown "+ "Touch.PrimaryTouchpad " + Time.time);
                    yield break;
                }

                //Debug.LogError ("2 ProcessMousePress "+ "PointerDown " + Time.time);
...
//            if (OVRInput.Get(OVRInput.Touch.PrimaryTouchpad)) {
//                //Debug.Log ("Process Get "+ " Touch.PrimaryTouchpad " + Time.time);
//                return;
//            }
...
//					print (graphicRect.gameObject.layer);
					if (graphicRect.gameObject.layer != 2)
					{
						//Set are gaze indicator with this world position and normal
						Vector3 worldPos = raycast.worldPosition;
						Vector3 normal = GetRectTransformNormal (graphicRect);
            ...
            if (physicsRaycaster)
            {
                ...
                {
					//print ("performSphereCastForGazepointer" + performSphereCastForGazepointer);
                ...
                //OVRGazePointer.instance.RequestShow();
//				print(physicsRaycaster.gameObject.name + "Physicsname");
//				OVRGazePointer.instance.RequestShow();
				if(raycast.gameObject.GetComponent<EventTrigger>() != null)
				{
...
//					OVRGazePointer.instance.SetPosition(position);
                ...
                {
//					print (raycast.worldNormal);
					//print ("matchNormalOnPhysicsColliders");
					OVRGazePointer.instance.SetPosition(position);
